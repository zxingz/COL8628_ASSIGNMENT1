...
class PacemakerDataset(Dataset):
    """Custom dataset for the Pacemaker data."""
...
if __name__ == "__main__":
    ...
    for folder in glob(f"{CONFIG['train_data_folder']}{os.sep}*"):
    ...
    # --- Data Preparation ---
    temp_train_dataset = PacemakerDataset(data=train_pacemaker_data)
    ...
    for variant in MODEL_VARIANTS.keys():
        ...
        test_dataset = PacemakerDataset(
            data=test_pacemaker_data,
            transform=transform,
            label_map=label_map
        )
        ...
        model_save_path = os.path.join(CONFIG["weights_folder"], f"task_2_1_best_vit_{variant}.pth")
        ...
        history_save_path = os.path.join(CONFIG["results_folder"], f'task_2_1_training_history_{variant}_{datetime.now().strftime("%Y%m%dT%H%M%S")}.json')
        ...
        results_path = os.path.join(CONFIG["results_folder"], f"task-2_1_{variant}.json")
